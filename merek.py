...
class MerekFunction(QWidget):
    merek_deleted = pyqtSignal(int)
    def __init__(self):

        super(MerekFunction, self).__init__()
        
        uic.loadUi("ui/merek.ui", self)
        self.merekTable.setColumnHidden(0, True)
        self.loadData()
        ...
        self.merekTable.itemSelectionChanged.connect(self.singleClick)
    ...
    def loadData(self):
        ...
        mydb.close()
        row = 0
        self.merekTable.setRowCount(len(self.myresult))
        for merek in self.myresult:
            self.merekTable.setItem(row, 0, QTableWidgetItem(str(merek[0])))
            self.merekTable.setItem(row, 1, QTableWidgetItem(merek[1]))
            row += 1
    ...
    def deleteWindow(self):
        self.singleClick()
        msgBox = QMessageBox()
        msgBox.setText("Apakah Anda yakin untuk menghapus merek ini ?")
        msgBox.setInformativeText("Nama Merek : "+" "+self.data[1])
        ...
        if msgBox.clickedButton() == buttonY:
            mydb = connection.Connect()
            mycursor = mydb.cursor()
            mycursor.execute("UPDATE merek SET deleted_At = %s WHERE id= %s ;", (datetime.now(timezone('Asia/Jakarta')), self.data[0]))
            mydb.commit()
            mycursor.close()
            mydb.close()
            msgBox1 = QMessageBox()
    ...
    def singleClick(self):  # enable edit button get item
        ...
        self.data = []
        row = self.merekTable.currentRow()
        for x in range(self.merekTable.columnCount()):
            self.data.insert(x, self.merekTable.item(row, x).text())
        return self.data
    ...
    def search(self):
        ...
        self.merekTable.clearContents()
        row = 0
        self.merekTable.setRowCount(len(myresult))

        for merek in myresult:
            self.merekTable.setItem(row, 0, QTableWidgetItem(str(merek[0])))
            self.merekTable.setItem(row, 1, QTableWidgetItem(merek[1]))
            row += 1
...
class Add(QDialog):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.setWindowTitle("Tambah Merek Barang")
        self.editLine.setPlaceholderText("Cth: Gratech")
    ...
    def Add_line_edit(self):
        ...
        line_edit.setPlaceholderText("Cth: Gratech")
    ...
    def Add_Action(self):
        ...
        mycursor = mydb.cursor()
        try:
            query = "INSERT INTO merek (nama_merek) VALUES (%s)"
...
class Edit(QDialog):
    def __init__(self, parent):
        ...
        self.setWindowTitle("Ubah Merek Barang")
        self.editLine.setPlaceholderText("Ubah Merek Barang")
    ...
    def Edit_Action(self, edit_array):
        ...
        mydb = connection.Connect()
        mycursor = mydb.cursor()
        query = "UPDATE merek SET nama_merek = %s WHERE id = %s "
